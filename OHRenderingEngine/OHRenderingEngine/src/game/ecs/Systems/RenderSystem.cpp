...
void RenderSystem::DrawNonTransparentObjects(const int * const indices, const Shader &shader)
{
	...
	while (indices[i] != -1)
	{
		...
		{
			float explodePower = 4 * abs(sin(glm::radians((explodeStartTime - time) * 300)));
...
void RenderSystem::UpdateModelMatrices()
{
	...
	for (int i = 0; i < Transform::m_Count; i++)
	{
		...
		model = glm::rotate(model, glm::radians(m_ECSManager->m_Transforms[i].m_RotationAngles.x), glm::vec3(1, 0, 0));
		model = glm::rotate(model, glm::radians(m_ECSManager->m_Transforms[i].m_RotationAngles.y), glm::vec3(0, 1, 0));
		model = glm::rotate(model, glm::radians(m_ECSManager->m_Transforms[i].m_RotationAngles.z), glm::vec3(0, 0, 1));
